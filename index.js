...
app.get("/products", (req, res) => {
  ...
  // { id : 2, showAvailable: 1 }
  const { q } = req.query;
  if (q) {
    const productsFilter = products.filter((product) =>
      product.name.toLowerCase().includes(q.toLowerCase())  // Samsung A33 (product)  SaMsUng (client)  ->  samsung a33 ¿se incluye? samsung 
    );
...
// Explicación -> El cliente hace la petición "localhost:3030/products/23218"  y en el lugar del idProduct obtenemos el numero 23218 ("string"), este valor nos sirve para hacer la búsqueda del producto.
// getProductById
